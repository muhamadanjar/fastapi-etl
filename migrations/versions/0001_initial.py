...
def upgrade() -> None:
    """Upgrade schema."""
    # A clean PostgreSQL database only contains the public schema. Provision the
    # service-owned namespaces before creating schema-qualified tables. IF NOT
    # EXISTS preserves compatibility with environments that provision them
    # outside Alembic.
    for schema_name in ETL_SCHEMAS:
...
def downgrade() -> None:
    ...
    # SQLAlchemy creates named PostgreSQL enum types separately from their
    # tables. Drop those owned types explicitly so downgrade-to-base followed by
    # a fresh upgrade does not fail with DuplicateObject. Schemas are retained:
    # a downgrade must not delete unrelated objects from shared namespaces.
